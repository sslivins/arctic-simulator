 ...
 *
 * Hardware: GC9107 128×128 RGB565 LCD
 *   Atom S3R: MOSI=21  CLK=15  CS=14  DC=42  RST=48  BL=I2C(0x30)
 *   Atom S3:  MOSI=21  CLK=17  CS=15  DC=33  RST=34  BL=GPIO 16
 *   BTN  = GPIO 41 (active-low, front-face button)
 *
 * Uses the esp_lcd ST7789 panel driver as structural wrapper for
 * draw_bitmap / set_gap, with a full GC9107 init sequence sent
 * manually (matching the M5GFX / LovyanGFX Panel_GC9107 driver).
 * Offset: x+0, y+32 (128×128 window in a 128×160 memory area).
 *
 * Backlight: Atom S3R uses an I2C LED controller at address 0x30
 * (SDA=45, SCL=0).  Atom S3 uses PWM on GPIO 16.  If I2C init fails
 * the driver falls back to GPIO (works for Atom S3 or custom setups).
 *
 * A 32 KB RGB565 framebuffer is drawn in software and flushed to the
 * panel via DMA.  The UI is intentionally minimal: status lines plus
 * a prominent recording indicator — perfect for the tiny 0.85" screen.
 */
#include "display.h"
...
static constexpr int CHAR_W  = 6;   // 5-pixel glyph + 1-pixel gap
static constexpr int CHAR_H  = 9;   // 7-pixel glyph + 2-pixel gap
static constexpr int COLS    = LCD_W / CHAR_W;  // 21
static constexpr int ROWS    = LCD_H / CHAR_H;  // 14

// ============================================================================
// RGB565 colours (byte-swapped for SPI big-endian)
// ============================================================================
static constexpr uint16_t swap16(uint16_t c) {
    return static_cast<uint16_t>(((c & 0xFF) << 8) | ((c >> 8) & 0xFF));
}

static constexpr uint16_t COL_BLACK     = swap16(0x0000);
...
static constexpr uint16_t COL_DARK_BLUE = swap16(0x0019);
static constexpr uint16_t COL_GRAY      = swap16(0x7BEF);
static constexpr uint16_t COL_DARK_GRAY = swap16(0x39E7);
static constexpr uint16_t COL_YELLOW    = swap16(0xFFE0);
static constexpr uint16_t COL_ORANGE    = swap16(0xFD20);

// ============================================================================
// 5×7 bitmap font (ASCII 32–126, 5 bytes per glyph, column-major, LSB=top)
// ============================================================================
// clang-format off
...
static void renderUI() {
    ...
    // ── Title bar ──────────────────────────────────────────────────────
    fbFillRect(0, y, LCD_W, CHAR_H, COL_DARK_BLUE);
    {
        char title[22];
        snprintf(title, sizeof(title), "ARCTIC SIM v%.5s", app->version);
        fbDrawStr(1, y + 1, title, COL_WHITE, COL_DARK_BLUE);
    }
    y += CHAR_H;

    // Separator
    fbHLine(0, y, LCD_W, COL_DARK_GRAY);
    y += 2;

    // ── Mode ───────────────────────────────────────────────────────────
    {
        auto pb = playback::getStatus();
        const char* mode_str = "Interactive";
        if (recorder::isRecording()) mode_str = "Recording";
        else if (pb.state == playback::State::PLAYING) mode_str = "Playback";
        else if (pb.state == playback::State::PAUSED)  mode_str = "Paused";

        char line[22];
        snprintf(line, sizeof(line), "Mode: %s", mode_str);
        fbDrawStr(1, y, line, COL_WHITE, COL_BLACK);
    }
    y += CHAR_H;
    ...
    // ── WiFi ───────────────────────────────────────────────────────────
    {
        char line[22];
        if (wifi::isConnected()) {
            snprintf(line, sizeof(line), "WiFi: %.14s", wifi::getIPAddress());
            fbDrawStr(1, y, line, COL_GREEN, COL_BLACK);
        } else {
            fbDrawStr(1, y, "WiFi: disconnected", COL_ORANGE, COL_BLACK);
        }
    }
    y += CHAR_H;
    ...
    // ── Modbus stats ───────────────────────────────────────────────────
    {
        auto st = mb_slave::getStats();
        char line[22];
        snprintf(line, sizeof(line), "MB R:%-5lu W:%-4lu",
                 (unsigned long)st.read_count, (unsigned long)st.write_count);
        uint16_t col = mb_slave::isInitialized() ? COL_WHITE : COL_DARK_GRAY;
        fbDrawStr(1, y, line, col, COL_BLACK);
    }
    y += CHAR_H;

    // ── Separator ──────────────────────────────────────────────────────
    y += 1;
    fbHLine(4, y, LCD_W - 8, COL_DARK_GRAY);
    y += 3;
    ...
    if (rec.recording) {
        // Red recording dot
        fbFillCircle(10, y + 3, 4, COL_RED);
        fbDrawStr(18, y, "RECORDING", COL_RED, COL_BLACK);
    } else {
        // Green square (ready indicator)
        fbFillRect(7, y, 7, 7, COL_GREEN);
        fbDrawStr(18, y, "READY", COL_GREEN, COL_BLACK);
    }
    y += CHAR_H;

    // Elapsed time
    {
        uint32_t secs = rec.elapsed_ms / 1000;
    ...
    {
        char line[22];
        snprintf(line, sizeof(line), "  %lu entries",
                 (unsigned long)rec.entries);
        fbDrawStr(1, y, line, COL_GRAY, COL_BLACK);
    }
    y += CHAR_H;
    ...
    // Storage usage
    {
        ...
        if (rec.bytes_total > 0) {
            float used_kb = (float)rec.bytes_used / 1024.0f;
            float total_kb = (float)rec.bytes_total / 1024.0f;
            if (total_kb >= 1024.0f) {
                snprintf(line, sizeof(line), "  %.0fKB/%.1fMB",
                         used_kb, total_kb / 1024.0f);
            } else {
                snprintf(line, sizeof(line), "  %.0f/%.0fKB",
                         used_kb, total_kb);
            }
        } else {
            snprintf(line, sizeof(line), "  No storage");
        }
        fbDrawStr(1, y, line, COL_GRAY, COL_BLACK);
    }
    y += CHAR_H;
    ...
    // ── Button hint ────────────────────────────────────────────────────
    fbDrawStr(1, y, "BTN: Toggle Rec", COL_DARK_GRAY, COL_BLACK);
}
